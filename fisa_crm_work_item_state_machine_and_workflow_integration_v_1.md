...
- Định nghĩa **state machine cho `work_items.status`** theo bộ trạng thái đã chốt (draft/open/in_progress/waiting_*/resolved/closed/canceled/rejected/archived).
...
- Áp dụng convention status ở DB là lowercase snake_case (`in_progress`, `waiting_internal`, ...).
...
**Bước 4 – Ghi lịch sử vào `public.work_item_state_history`**
...
**(Tuỳ chọn) Bước 5 – Ghi outbox event `WORK_ITEM_STATUS_CHANGED`**
...
**`IWorkflowRuntimeClient`** là abstraction gói việc:
...
**Loose coupling (`allow_post_close_work = true`)**:
...
1. **Domain/Application**
   - [ ] Tạo enum `WorkItemAction`.
   - [ ] Tạo class `WorkItemActionContext`, `WorkItemStateChangeResult`.
   - [ ] Tạo interface `IWorkItemStateMachine`.
   - [ ] Implement `WorkItemStateMachine` (theo pseudo-code + bảng transition).

2. **Application Service**
   - [ ] Tạo `IWorkItemAppService` + `WorkItemAppService` với method `ApplyActionAsync(...)`.
   - [ ] Implement mapping `actionCode` string → `WorkItemAction`.
   - [ ] Inject & gọi `IWorkItemStateMachine` trong `WorkItemAppService`.
   - [ ] Tạo interface `IWorkflowRuntimeClient` để gọi sang Workflow Engine.

3. **API Layer**
   - [ ] Tạo endpoint `POST /api/crm/work-items/{id}/actions`.
   - [ ] Định nghĩa DTO `ApplyWorkItemActionRequest`.
   - [ ] Map các nút trên UI (Close, Resolve, Wait Internal, Wait Customer, Reopen, ...) → action code string.

4. **Workflow Engine Runtime v1**
   - [ ] Tại chỗ kết thúc instance (step cuối): thay `UPDATE work_items` trực tiếp bằng gọi `IWorkItemStateMachine.ApplyActionAsync(..., AutoCloseFromWorkflow, ...)`.
   - [ ] Đảm bảo dùng chung transaction khi update workflow + work item.
   - [ ] (Tuỳ chọn) Expose method ở `IWorkflowRuntimeClient` để CRM có thể gọi khi người dùng Close ticket.

5. **Worker / Outbox**
   - [ ] Kiểm tra luồng `WORK_ITEM_CREATED` không bị ảnh hưởng.
   - [ ] (Optional) Thêm event `WORK_ITEM_STATUS_CHANGED` sau khi state machine chạy xong.

6. **Test**
   - [ ] Viết unit test cho `IsTransitionAllowed` & `GetNextStatus`.
   - [ ] Viết integration test cho endpoint `/actions` với các case chính: Submit, StartWork, Wait, Resolve, Close, Cancel, Reopen.
   - [ ] Test integration giữa CRM & Workflow Engine trong strong coupling:
     - Close từ CRM → instance completed.
     - Instance completed từ Workflow → WI closed.
...
2. **Khi người dùng bấm "Gửi yêu cầu"**
   ...
   - Application Service bên trong sẽ:
     ...
     - Gọi **state machine** với:
       ...
       - Sau đó `action = WorkItemAction.Submit` để đưa trạng thái **`draft → open`**.
   - Kết quả:
...
> Tóm lại: **màn Tạo yêu cầu chỉ có 1 điểm chạm với state machine: nút "Gửi yêu cầu" → action `Submit` (sau khi tạo).**
...
1. **Nếu status hiện tại là `resolved`** (đã xử lý xong):
   - "Đóng" = **Close** chuẩn.
   - FE gọi:
...
2. **Nếu status hiện tại là `draft`, `open`, `in_progress`, hoặc `waiting_*`**:
...
3. **Nếu status hiện tại là `closed`, `canceled`, `rejected`, `archived`**:
...
Nhờ mapping này, dev FE/BE nhìn UI là biết **khi nào phải gọi `/actions` (state machine)** và khi nào chỉ gọi API update dữ liệu thường, tránh lẫn lộn giữa update field và chuyển trạng thái nghiệp vụ.
...
Tài liệu này chuẩn hoá danh sách giá trị **string** (lowercase, snake_case nếu nhiều từ):
